...
7. **Classify every top-level file/dir as LIVE / LEGACY / UNCLEAR before Phase 2** (PROJECT OVERRIDE — required for this repo):
   - **LIVE**: reachable from entry points declared in `render.yaml` `startCommand`, `Procfile`, `Dockerfile`, or `package.json` scripts. Build the import graph from each entry point.
   - **LEGACY**: any of:
     - Path matches `raw_recovery/`, `historical/`, `*.bak`, `legacy/`, `old/`
     - Explicitly marked deprecated in `README.md`
     - Not reachable from entry points AND not modified in 6+ months (per Phase 1 step 3 git log data)
   - **UNCLEAR**: anything that doesn't fit the two above. Flag in Open Questions.

   Write the classification to `AUDIT_GROUND_TRUTH.md` in repo root as a one-page table with columns `Path | Class | Reason`. Include this same table as an appendix in `TECH_DEBT_AUDIT.md`.
...
**PROJECT OVERRIDE — LEGACY handling:** Skip files classified as LEGACY in Phase 1 step 7 unless the finding has security implications (hardcoded secret, exposed endpoint, leaked credential). For LEGACY files with security findings, prefix the Description column with `[LEGACY]` — the recommendation should be deletion of the file, not refactor. Don't generate findings for LEGACY files in dimensions like Architectural decay, Consistency rot, Type debt, Performance, or Documentation drift.
