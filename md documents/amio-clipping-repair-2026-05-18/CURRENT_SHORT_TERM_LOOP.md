...
Every output in this loop must start with the updated Mandatory Output Anchor
in `LOOP_OPERATING_PROTOCOL.md`. If a barrier appears, the agent must answer
it, write it to `WORK_LOG.md`, re-read the long-term goals or recent logs when
in doubt, and continue with another useful step instead of ending the turn.
