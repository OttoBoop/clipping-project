...
## 2026-06-05 15:04:06 -03 - Backend Low-Count Review: Google News Blocking Source Runs

- Trigger: user correctly flagged that the current viewer-visible/news count is too low for 18 broad targets over 2014-01-01 to 2026-06-02.
- Production status check: job `0b36e332911a` remains the only job in use; status `running`, coverage `running`, source-run counts `{"complete": 18, "running": 1, "pending": 22892}`.
- Grouped ledger contract is intact: `sourceRunTargetCounts={"__all_targets__": 22911}` and source types `{"rss": 18, "google_news": 1, "wordpress_api": 6, "internal_search": 6, "sitemap_daily": 22680, "vejario_archive": 100, "camara_archive": 100}`.
- Current barrier: grouped `Google News` source-run started at `2026-06-05T17:45:59Z` and remained the active source with stale progress `seen=3`, `total=3`, `articles=87`, `mentions=116`, `stories=87`, `publishedAt=2026-06-05T17:45:26.523431Z`.
- Backend diagnosis: prior grouped Google News circuit breaker was placed after `collect_google_news()`. It prevented full ingestion but still allowed the grouped Google collection phase to block before the job could reach WordPress, internal search, and sitemap source-runs.
- Local fix prepared: `collect_source_run_candidates()` now immediately returns no candidates and `complete=True` for grouped `google_news` rows before calling `collect_google_news()`.
- Test tightened: `test_grouped_google_news_source_run_circuit_breaker_skips_ingest` now fails if `collect_google_news()` is called for grouped Google News and asserts zero candidates saved/seen.
- Regression tests: `.venv_playwright/bin/python -m pytest tests/test_targets_jobs.py tests/test_voluntarios_backfill_operator.py -q` passed with `87 passed`.
- Next action: deploy this backend fix, resume only job `0b36e332911a`, verify grouped Google completes immediately, then monitor WordPress and sitemap progress for real article/story growth.

## 2026-06-05 15:12:46 -03 - Backend Low-Count Review: Matcher Accent/Punctuation Loss

- Production target verification: all 18 expected profile targets are active primary targets with `className="primary"`.
- Target keyword contract check found accent variants for most accented terms, but `seguranca_presente` only had `["Segurança Presente"]` in `/api/targets`; without accent-insensitive matching, text containing `Seguranca Presente` could be missed.
- Backend diagnosis: `pipeline.matcher.CitationMatcher` normalized only with `.lower()`. It did not fold diacritics or punctuation, so target phrases could miss unaccented text or slug/hyphen text such as `Seguranca-Presente` and `percepcao-de-seguranca`.
- Local fix prepared: `CitationMatcher` now normalizes both keywords and article text with `casefold`, diacritic stripping, punctuation-to-space folding, and whitespace collapse while preserving the original `keyword_matched` value.
- Test added: `tests/test_matcher.py::test_citation_matcher_is_accent_and_punctuation_insensitive`.
- Regression tests: `.venv_playwright/bin/python -m pytest tests/test_matcher.py tests/test_ingest_restore.py tests/test_wave2_pipeline_restore.py tests/test_targets_jobs.py tests/test_voluntarios_backfill_operator.py -q` passed with `119 passed`.
- Production before this deploy: resumed grouped job was saving again on RSS (`G1 Rio`) with `sourceRunCounts={"complete": 7, "running": 1, "pending": 22903}` and `publishedAt=2026-06-05T18:12:16.445822Z`, but it was still running with the old matcher.
- Next action: deploy matcher normalization, then resume only job `0b36e332911a` so the 2014 backfill runs with accent/punctuation-insensitive matching.
...
## 2026-06-05 11:21:18 -03 - Resumed Same Production Job
...
- Current target/source: `n/a` / `n/a`.
- Totals/status fields: articles `None`, mentions `805`, stories `301`, publishedAt `2026-06-03T01:40:39.522579+00:00`.
- Website asset: generatedAt `05/06/2026 14:12 UTC`, stories `1437`, articles `1895`, targets `27`.
- Live results: latest `2026-06-03T16:51:31.525271+00:00`, count `51`.
- Viewer profile/login: login ok `True`, profile `{'found': True, 'label': 'Voluntários-Lab-Políticas-Públicas', 'hasPassword': True, 'targetKeysCount': 18, 'defaultTargetsCount': 18, 'missing': [], 'extra': [], 'defaultsMissing': [], 'defaultsExtra': [], 'targetKeys': ['seguranca_presente', 'programa_seguranca_presente', 'operacao_seguranca_presente', 'seguranca', 'inseguranca', 'crime', 'criminalidade', 'violencia', 'assalto', 'roubo', 'furto', 'medo', 'policiamento', 'patrulhamento', 'percepcao_de_seguranca', 'sensacao_de_seguranca', 'reforco_no_policiamento', 'ordem_publica'], 'defaultTargets': ['seguranca_presente', 'programa_seguranca_presente', 'operacao_seguranca_presente', 'seguranca', 'inseguranca', 'crime', 'criminalidade', 'violencia', 'assalto', 'roubo', 'furto', 'medo', 'policiamento', 'patrulhamento', 'percepcao_de_seguranca', 'sensacao_de_seguranca', 'reforco_no_policiamento', 'ordem_publica']}`.
- Target contract: `{'count': 18, 'missing': [], 'extra': [], 'primaryExact': True}`.
- Memory: VmRSS `179.68` MiB, VmHWM `682.72` MiB, limit `512` MiB.
- Disk: free `50717.38` MiB, DB `254.17` MiB.
- Storage diagnostic: enabled `False`, reason `supabase_env_not_available_locally`.
- HTTP timings/status: `{"asset": {"elapsed": 0.58, "status": 200}, "disk": {"elapsed": 0.24, "status": 200}, "events": {"elapsed": 0.22, "status": 200}, "health": {"elapsed": 1.31, "status": 200}, "live": {"elapsed": 0.28, "status": 200}, "memory": {"elapsed": 0.22, "status": 200}, "sqlite": {"elapsed": 0.63, "status": 200}, "status": {"elapsed": 1.32, "status": 200}, "targets": {"elapsed": 0.24, "status": 200}, "viewers": {"elapsed": 0.22, "status": 200}}`.
- Recent source events: `[{"at": "2026-06-03T01:41:53.755590+00:00", "event": "source_run_started", "target": "seguranca_presente", "source": "Veja Rio", "status": "running", "candidatesSeen": null, "candidatesTotal": null, "articlesInserted": null, "mentionsInserted": null, "storiesTouched": null, "rssBefore": 239.91, "rssAfter": null, "error": ""}, {"at": "2026-06-03T01:41:40.247251+00:00", "event": "source_run_checkpoint", "target": "seguranca_presente", "source": "Veja Rio", "status": "pending", "candidatesSeen": 25, "candidatesTotal": 25, "articlesInserted": 4, "mentionsInserted": 4, "storiesTouched": 4, "rssBefore": 239.91, "rssAfter": 239.91, "error": ""}, {"at": "2026-06-03T01:40:52.964517+00:00", "event": "source_run_started", "target": "seguranca_presente", "source": "Veja Rio", "status": "running", "candidatesSeen": null, "candidatesTotal": null, "articlesInserted": null, "mentionsInserted": null, "storiesTouched": null, "rssBefore": 239.91, "rssAfter": null, "error": ""}, {"at": "2026-06-03T01:40:22.446931+00:00", "event": "source_run_checkpoint", "target": "seguranca_presente", "source": "Veja Rio", "status": "pending", "candidatesSeen": 25, "candidatesTotal": 25, "articlesInserted": 10, "mentionsInserted": 10, "storiesTouched": 10, "rssBefore": 239.91, "rssAfter": 239.91, "error": ""}, {"at": "2026-06-03T01:39:08.444608+00:00", "event": "source_run_started", "target": "seguranca_presente", "source": "Veja Rio", "status": "running", "candidatesSeen": null, "candidatesTotal": null, "articlesInserted": null, "mentionsInserted": null, "storiesTouched": null, "rssBefore": 239.91, "rssAfter": null, "error": ""}, {"at": "2026-06-03T01:38:38.069488+00:00", "event": "source_run_complete", "target": "seguranca_presente", "source": "Tribuna da Serra", "status": "complete", "candidatesSeen": 24, "candidatesTotal": 24, "articlesInserted": 8, "mentionsInserted": 8, "storiesTouched": 8, "rssBefore": 239.91, "rssAfter": 239.91, "error": ""}, {"at": "2026-06-03T01:36:57.604497+00:00", "event": "source_run_started", "target": "seguranca_presente", "source": "Tribuna da Serra", "status": "running", "candidatesSeen": null, "candidatesTotal": null, "articlesInserted": null, "mentionsInserted": null, "storiesTouched": null, "rssBefore": 239.91, "rssAfter": null, "error": ""}, {"at": "2026-06-03T01:36:25.606763+00:00", "event": "source_run_checkpoint", "target": "seguranca_presente", "source": "Tribuna da Serra", "status": "pending", "candidatesSeen": 25, "candidatesTotal": 25, "articlesInserted": 4, "mentionsInserted": 4, "storiesTouched": 4, "rssBefore": 239.91, "rssAfter": 239.91, "error": ""}]`.
- Barriers: `[]`.

## 2026-06-05 11:22:41 -03 - Monitor Cycle 1

- Job `0b36e332911a` status `running`, coverage `running`, resumeAvailable `False`.
- Contract: date `2014-01-01` to `2026-06-02`, collector `all`, preset `custom`, exact 18 keys `True`.
- Source ledger: total `22911`, counts `{"complete": 6, "pending": 22904, "running": 1}`.
- Current target/source: `n/a` / `n/a`.
- Totals/status fields: articles `864`, mentions `1302`, stories `1302`, publishedAt `2026-06-03T01:40:39.522579+00:00`.
- Website asset: generatedAt `05/06/2026 14:12 UTC`, stories `1437`, articles `1895`, targets `27`.
- Live results: latest `2026-06-05T14:22:28.617943+00:00`, count `51`.
- Viewer profile/login: login ok `True`, profile `{'found': True, 'label': 'Voluntários-Lab-Políticas-Públicas', 'hasPassword': True, 'targetKeysCount': 18, 'defaultTargetsCount': 18, 'missing': [], 'extra': [], 'defaultsMissing': [], 'defaultsExtra': [], 'targetKeys': ['seguranca_presente', 'programa_seguranca_presente', 'operacao_seguranca_presente', 'seguranca', 'inseguranca', 'crime', 'criminalidade', 'violencia', 'assalto', 'roubo', 'furto', 'medo', 'policiamento', 'patrulhamento', 'percepcao_de_seguranca', 'sensacao_de_seguranca', 'reforco_no_policiamento', 'ordem_publica'], 'defaultTargets': ['seguranca_presente', 'programa_seguranca_presente', 'operacao_seguranca_presente', 'seguranca', 'inseguranca', 'crime', 'criminalidade', 'violencia', 'assalto', 'roubo', 'furto', 'medo', 'policiamento', 'patrulhamento', 'percepcao_de_seguranca', 'sensacao_de_seguranca', 'reforco_no_policiamento', 'ordem_publica']}`.
- Target contract: `{'count': 18, 'missing': [], 'extra': [], 'primaryExact': True}`.
- Memory: VmRSS `206.33` MiB, VmHWM `682.72` MiB, limit `512` MiB.
- Disk: free `50141.87` MiB, DB `269.06` MiB.
- Storage diagnostic: enabled `False`, reason `supabase_env_not_available_locally`.
- HTTP timings/status: `{"asset": {"elapsed": 0.57, "status": 200}, "disk": {"elapsed": 0.26, "status": 200}, "events": {"elapsed": 0.23, "status": 200}, "health": {"elapsed": 2.26, "status": 200}, "live": {"elapsed": 0.34, "status": 200}, "memory": {"elapsed": 0.36, "status": 200}, "sqlite": {"elapsed": 0.94, "status": 200}, "status": {"elapsed": 1.84, "status": 200}, "targets": {"elapsed": 0.23, "status": 200}, "viewers": {"elapsed": 0.23, "status": 200}}`.
- Recent source events: `[{"at": "2026-06-05T14:22:13.054868+00:00", "event": "source_run_started", "target": "__all_targets__", "source": "G1 Politica", "status": "running", "candidatesSeen": null, "candidatesTotal": null, "articlesInserted": null, "mentionsInserted": null, "storiesTouched": null, "rssBefore": 195.77, "rssAfter": null, "error": ""}, {"at": "2026-06-05T14:22:12.930949+00:00", "event": "source_run_complete", "target": "__all_targets__", "source": "G1", "status": "complete", "candidatesSeen": 0, "candidatesTotal": 0, "articlesInserted": 0, "mentionsInserted": 0, "storiesTouched": 0, "rssBefore": 194.36, "rssAfter": 195.77, "error": ""}, {"at": "2026-06-05T14:22:11.297682+00:00", "event": "source_run_started", "target": "__all_targets__", "source": "G1", "status": "running", "candidatesSeen": null, "candidatesTotal": null, "articlesInserted": null, "mentionsInserted": null, "storiesTouched": null, "rssBefore": 194.36, "rssAfter": null, "error": ""}, {"at": "2026-06-05T14:22:11.145279+00:00", "event": "source_run_complete", "target": "__all_targets__", "source": "Veja Rio", "status": "complete", "candidatesSeen": 0, "candidatesTotal": 0, "articlesInserted": 0, "mentionsInserted": 0, "storiesTouched": 0, "rssBefore": 193.43, "rssAfter": 194.36, "error": ""}, {"at": "2026-06-05T14:22:10.901594+00:00", "event": "source_run_started", "target": "__all_targets__", "source": "Veja Rio", "status": "running", "candidatesSeen": null, "candidatesTotal": null, "articlesInserted": null, "mentionsInserted": null, "storiesTouched": null, "rssBefore": 193.43, "rssAfter": null, "error": ""}, {"at": "2026-06-05T14:22:10.750875+00:00", "event": "source_run_complete", "target": "__all_targets__", "source": "VEJA Cidades", "status": "complete", "candidatesSeen": 0, "candidatesTotal": 0, "articlesInserted": 0, "mentionsInserted": 0, "storiesTouched": 0, "rssBefore": 194.43, "rssAfter": 194.43, "error": ""}, {"at": "2026-06-05T14:22:10.635389+00:00", "event": "source_run_started", "target": "__all_targets__", "source": "VEJA Cidades", "status": "running", "candidatesSeen": null, "candidatesTotal": null, "articlesInserted": null, "mentionsInserted": null, "storiesTouched": null, "rssBefore": 194.43, "rssAfter": null, "error": ""}, {"at": "2026-06-05T14:22:10.489977+00:00", "event": "source_run_complete", "target": "__all_targets__", "source": "VEJA Politica", "status": "complete", "candidatesSeen": 0, "candidatesTotal": 0, "articlesInserted": 0, "mentionsInserted": 0, "storiesTouched": 0, "rssBefore": 195.3, "rssAfter": 194.43, "error": ""}]`.
- Barriers: `[]`.

## 2026-06-05 11:23:54 -03 - Monitor Cycle 2

- Job `0b36e332911a` status `running`, coverage `running`, resumeAvailable `False`.
- Contract: date `2014-01-01` to `2026-06-02`, collector `all`, preset `custom`, exact 18 keys `True`.
- Source ledger: total `22911`, counts `{"complete": 6, "pending": 22904, "running": 1}`.
- Current target/source: `n/a` / `n/a`.
- Totals/status fields: articles `874`, mentions `1314`, stories `1312`, publishedAt `2026-06-03T01:40:39.522579+00:00`.
- Website asset: generatedAt `05/06/2026 14:12 UTC`, stories `1437`, articles `1895`, targets `27`.
- Live results: latest `2026-06-05T14:23:36.906759+00:00`, count `57`.
- Viewer profile/login: login ok `True`, profile `{'found': True, 'label': 'Voluntários-Lab-Políticas-Públicas', 'hasPassword': True, 'targetKeysCount': 18, 'defaultTargetsCount': 18, 'missing': [], 'extra': [], 'defaultsMissing': [], 'defaultsExtra': [], 'targetKeys': ['seguranca_presente', 'programa_seguranca_presente', 'operacao_seguranca_presente', 'seguranca', 'inseguranca', 'crime', 'criminalidade', 'violencia', 'assalto', 'roubo', 'furto', 'medo', 'policiamento', 'patrulhamento', 'percepcao_de_seguranca', 'sensacao_de_seguranca', 'reforco_no_policiamento', 'ordem_publica'], 'defaultTargets': ['seguranca_presente', 'programa_seguranca_presente', 'operacao_seguranca_presente', 'seguranca', 'inseguranca', 'crime', 'criminalidade', 'violencia', 'assalto', 'roubo', 'furto', 'medo', 'policiamento', 'patrulhamento', 'percepcao_de_seguranca', 'sensacao_de_seguranca', 'reforco_no_policiamento', 'ordem_publica']}`.
- Target contract: `{'count': 18, 'missing': [], 'extra': [], 'primaryExact': True}`.
- Memory: VmRSS `208.41` MiB, VmHWM `682.72` MiB, limit `512` MiB.
- Disk: free `50239.25` MiB, DB `269.06` MiB.
- Storage diagnostic: enabled `False`, reason `supabase_env_not_available_locally`.
- HTTP timings/status: `{"asset": {"elapsed": 0.5, "status": 200}, "disk": {"elapsed": 0.23, "status": 200}, "events": {"elapsed": 0.23, "status": 200}, "health": {"elapsed": 1.84, "status": 200}, "live": {"elapsed": 0.39, "status": 200}, "memory": {"elapsed": 0.7, "status": 200}, "sqlite": {"elapsed": 0.59, "status": 200}, "status": {"elapsed": 1.67, "status": 200}, "targets": {"elapsed": 0.24, "status": 200}, "viewers": {"elapsed": 0.23, "status": 200}}`.
- Recent source events: `[{"at": "2026-06-05T14:22:13.054868+00:00", "event": "source_run_started", "target": "__all_targets__", "source": "G1 Politica", "status": "running", "candidatesSeen": null, "candidatesTotal": null, "articlesInserted": null, "mentionsInserted": null, "storiesTouched": null, "rssBefore": 195.77, "rssAfter": null, "error": ""}, {"at": "2026-06-05T14:22:12.930949+00:00", "event": "source_run_complete", "target": "__all_targets__", "source": "G1", "status": "complete", "candidatesSeen": 0, "candidatesTotal": 0, "articlesInserted": 0, "mentionsInserted": 0, "storiesTouched": 0, "rssBefore": 194.36, "rssAfter": 195.77, "error": ""}, {"at": "2026-06-05T14:22:11.297682+00:00", "event": "source_run_started", "target": "__all_targets__", "source": "G1", "status": "running", "candidatesSeen": null, "candidatesTotal": null, "articlesInserted": null, "mentionsInserted": null, "storiesTouched": null, "rssBefore": 194.36, "rssAfter": null, "error": ""}, {"at": "2026-06-05T14:22:11.145279+00:00", "event": "source_run_complete", "target": "__all_targets__", "source": "Veja Rio", "status": "complete", "candidatesSeen": 0, "candidatesTotal": 0, "articlesInserted": 0, "mentionsInserted": 0, "storiesTouched": 0, "rssBefore": 193.43, "rssAfter": 194.36, "error": ""}, {"at": "2026-06-05T14:22:10.901594+00:00", "event": "source_run_started", "target": "__all_targets__", "source": "Veja Rio", "status": "running", "candidatesSeen": null, "candidatesTotal": null, "articlesInserted": null, "mentionsInserted": null, "storiesTouched": null, "rssBefore": 193.43, "rssAfter": null, "error": ""}, {"at": "2026-06-05T14:22:10.750875+00:00", "event": "source_run_complete", "target": "__all_targets__", "source": "VEJA Cidades", "status": "complete", "candidatesSeen": 0, "candidatesTotal": 0, "articlesInserted": 0, "mentionsInserted": 0, "storiesTouched": 0, "rssBefore": 194.43, "rssAfter": 194.43, "error": ""}, {"at": "2026-06-05T14:22:10.635389+00:00", "event": "source_run_started", "target": "__all_targets__", "source": "VEJA Cidades", "status": "running", "candidatesSeen": null, "candidatesTotal": null, "articlesInserted": null, "mentionsInserted": null, "storiesTouched": null, "rssBefore": 194.43, "rssAfter": null, "error": ""}, {"at": "2026-06-05T14:22:10.489977+00:00", "event": "source_run_complete", "target": "__all_targets__", "source": "VEJA Politica", "status": "complete", "candidatesSeen": 0, "candidatesTotal": 0, "articlesInserted": 0, "mentionsInserted": 0, "storiesTouched": 0, "rssBefore": 195.3, "rssAfter": 194.43, "error": ""}]`.
- Barriers: `[]`.

## 2026-06-05 11:25:13 -03 - Monitor Cycle 3

- Job `0b36e332911a` status `running`, coverage `pending`, resumeAvailable `False`.
- Contract: date `2014-01-01` to `2026-06-02`, collector `all`, preset `custom`, exact 18 keys `True`.
- Source ledger: total `22911`, counts `{"complete": 7, "pending": 22904}`.
- Current target/source: `n/a` / `n/a`.
- Totals/status fields: articles `21`, mentions `26`, stories `21`, publishedAt `2026-06-05T14:24:59.252338+00:00`.
- Website asset: generatedAt `05/06/2026 14:24 UTC`, stories `1437`, articles `1893`, targets `27`.
- Live results: latest `2026-06-05T14:24:38.782354+00:00`, count `60`.
- Viewer profile/login: login ok `True`, profile `{'found': True, 'label': 'Voluntários-Lab-Políticas-Públicas', 'hasPassword': True, 'targetKeysCount': 18, 'defaultTargetsCount': 18, 'missing': [], 'extra': [], 'defaultsMissing': [], 'defaultsExtra': [], 'targetKeys': ['seguranca_presente', 'programa_seguranca_presente', 'operacao_seguranca_presente', 'seguranca', 'inseguranca', 'crime', 'criminalidade', 'violencia', 'assalto', 'roubo', 'furto', 'medo', 'policiamento', 'patrulhamento', 'percepcao_de_seguranca', 'sensacao_de_seguranca', 'reforco_no_policiamento', 'ordem_publica'], 'defaultTargets': ['seguranca_presente', 'programa_seguranca_presente', 'operacao_seguranca_presente', 'seguranca', 'inseguranca', 'crime', 'criminalidade', 'violencia', 'assalto', 'roubo', 'furto', 'medo', 'policiamento', 'patrulhamento', 'percepcao_de_seguranca', 'sensacao_de_seguranca', 'reforco_no_policiamento', 'ordem_publica']}`.
- Target contract: `{'count': 18, 'missing': [], 'extra': [], 'primaryExact': True}`.
- Memory: VmRSS `223.44` MiB, VmHWM `682.72` MiB, limit `512` MiB.
- Disk: free `49911.75` MiB, DB `271.8` MiB.
- Storage diagnostic: enabled `False`, reason `supabase_env_not_available_locally`.
- HTTP timings/status: `{"asset": {"elapsed": 0.81, "status": 200}, "disk": {"elapsed": 0.41, "status": 200}, "events": {"elapsed": 0.23, "status": 200}, "health": {"elapsed": 2.05, "status": 200}, "live": {"elapsed": 0.45, "status": 200}, "memory": {"elapsed": 0.45, "status": 200}, "sqlite": {"elapsed": 1.3, "status": 200}, "status": {"elapsed": 2.21, "status": 200}, "targets": {"elapsed": 0.22, "status": 200}, "viewers": {"elapsed": 0.23, "status": 200}}`.
- Recent source events: `[{"at": "2026-06-05T14:24:40.275724+00:00", "event": "source_run_complete", "target": "__all_targets__", "source": "G1 Politica", "status": "complete", "candidatesSeen": 54, "candidatesTotal": 54, "articlesInserted": 21, "mentionsInserted": 26, "storiesTouched": 21, "rssBefore": 195.77, "rssAfter": 209.25, "error": ""}, {"at": "2026-06-05T14:22:13.054868+00:00", "event": "source_run_started", "target": "__all_targets__", "source": "G1 Politica", "status": "running", "candidatesSeen": null, "candidatesTotal": null, "articlesInserted": null, "mentionsInserted": null, "storiesTouched": null, "rssBefore": 195.77, "rssAfter": null, "error": ""}, {"at": "2026-06-05T14:22:12.930949+00:00", "event": "source_run_complete", "target": "__all_targets__", "source": "G1", "status": "complete", "candidatesSeen": 0, "candidatesTotal": 0, "articlesInserted": 0, "mentionsInserted": 0, "storiesTouched": 0, "rssBefore": 194.36, "rssAfter": 195.77, "error": ""}, {"at": "2026-06-05T14:22:11.297682+00:00", "event": "source_run_started", "target": "__all_targets__", "source": "G1", "status": "running", "candidatesSeen": null, "candidatesTotal": null, "articlesInserted": null, "mentionsInserted": null, "storiesTouched": null, "rssBefore": 194.36, "rssAfter": null, "error": ""}, {"at": "2026-06-05T14:22:11.145279+00:00", "event": "source_run_complete", "target": "__all_targets__", "source": "Veja Rio", "status": "complete", "candidatesSeen": 0, "candidatesTotal": 0, "articlesInserted": 0, "mentionsInserted": 0, "storiesTouched": 0, "rssBefore": 193.43, "rssAfter": 194.36, "error": ""}, {"at": "2026-06-05T14:22:10.901594+00:00", "event": "source_run_started", "target": "__all_targets__", "source": "Veja Rio", "status": "running", "candidatesSeen": null, "candidatesTotal": null, "articlesInserted": null, "mentionsInserted": null, "storiesTouched": null, "rssBefore": 193.43, "rssAfter": null, "error": ""}, {"at": "2026-06-05T14:22:10.750875+00:00", "event": "source_run_complete", "target": "__all_targets__", "source": "VEJA Cidades", "status": "complete", "candidatesSeen": 0, "candidatesTotal": 0, "articlesInserted": 0, "mentionsInserted": 0, "storiesTouched": 0, "rssBefore": 194.43, "rssAfter": 194.43, "error": ""}, {"at": "2026-06-05T14:22:10.635389+00:00", "event": "source_run_started", "target": "__all_targets__", "source": "VEJA Cidades", "status": "running", "candidatesSeen": null, "candidatesTotal": null, "articlesInserted": null, "mentionsInserted": null, "storiesTouched": null, "rssBefore": 194.43, "rssAfter": null, "error": ""}]`.
- Barriers: `[]`.

## 2026-06-05 11:26:23 -03 - Monitor Cycle 4

- Job `0b36e332911a` status `running`, coverage `running`, resumeAvailable `False`.
- Contract: date `2014-01-01` to `2026-06-02`, collector `all`, preset `custom`, exact 18 keys `True`.
- Source ledger: total `22911`, counts `{"complete": 7, "pending": 22903, "running": 1}`.
- Current target/source: `n/a` / `n/a`.
- Totals/status fields: articles `892`, mentions `1339`, stories `1330`, publishedAt `2026-06-05T14:24:59.252338+00:00`.
- Website asset: generatedAt `05/06/2026 14:24 UTC`, stories `1437`, articles `1893`, targets `27`.
- Live results: latest `2026-06-05T14:26:21.198715+00:00`, count `60`.
- Viewer profile/login: login ok `True`, profile `{'found': True, 'label': 'Voluntários-Lab-Políticas-Públicas', 'hasPassword': True, 'targetKeysCount': 18, 'defaultTargetsCount': 18, 'missing': [], 'extra': [], 'defaultsMissing': [], 'defaultsExtra': [], 'targetKeys': ['seguranca_presente', 'programa_seguranca_presente', 'operacao_seguranca_presente', 'seguranca', 'inseguranca', 'crime', 'criminalidade', 'violencia', 'assalto', 'roubo', 'furto', 'medo', 'policiamento', 'patrulhamento', 'percepcao_de_seguranca', 'sensacao_de_seguranca', 'reforco_no_policiamento', 'ordem_publica'], 'defaultTargets': ['seguranca_presente', 'programa_seguranca_presente', 'operacao_seguranca_presente', 'seguranca', 'inseguranca', 'crime', 'criminalidade', 'violencia', 'assalto', 'roubo', 'furto', 'medo', 'policiamento', 'patrulhamento', 'percepcao_de_seguranca', 'sensacao_de_seguranca', 'reforco_no_policiamento', 'ordem_publica']}`.
- Target contract: `{'count': 18, 'missing': [], 'extra': [], 'primaryExact': True}`.
- Memory: VmRSS `224.01` MiB, VmHWM `682.72` MiB, limit `512` MiB.
- Disk: free `50143.72` MiB, DB `271.8` MiB.
- Storage diagnostic: enabled `False`, reason `supabase_env_not_available_locally`.
- HTTP timings/status: `{"asset": {"elapsed": 0.5, "status": 200}, "disk": {"elapsed": 0.24, "status": 200}, "events": {"elapsed": 0.23, "status": 200}, "health": {"elapsed": 1.04, "status": 200}, "live": {"elapsed": 0.31, "status": 200}, "memory": {"elapsed": 0.22, "status": 200}, "sqlite": {"elapsed": 0.62, "status": 200}, "status": {"elapsed": 1.44, "status": 200}, "targets": {"elapsed": 0.22, "status": 200}, "viewers": {"elapsed": 0.22, "status": 200}}`.
- Recent source events: `[{"at": "2026-06-05T14:25:14.916917+00:00", "event": "source_run_started", "target": "__all_targets__", "source": "G1 Rio", "status": "running", "candidatesSeen": null, "candidatesTotal": null, "articlesInserted": null, "mentionsInserted": null, "storiesTouched": null, "rssBefore": 221.79, "rssAfter": null, "error": ""}, {"at": "2026-06-05T14:24:40.275724+00:00", "event": "source_run_complete", "target": "__all_targets__", "source": "G1 Politica", "status": "complete", "candidatesSeen": 54, "candidatesTotal": 54, "articlesInserted": 21, "mentionsInserted": 26, "storiesTouched": 21, "rssBefore": 195.77, "rssAfter": 209.25, "error": ""}, {"at": "2026-06-05T14:22:13.054868+00:00", "event": "source_run_started", "target": "__all_targets__", "source": "G1 Politica", "status": "running", "candidatesSeen": null, "candidatesTotal": null, "articlesInserted": null, "mentionsInserted": null, "storiesTouched": null, "rssBefore": 195.77, "rssAfter": null, "error": ""}, {"at": "2026-06-05T14:22:12.930949+00:00", "event": "source_run_complete", "target": "__all_targets__", "source": "G1", "status": "complete", "candidatesSeen": 0, "candidatesTotal": 0, "articlesInserted": 0, "mentionsInserted": 0, "storiesTouched": 0, "rssBefore": 194.36, "rssAfter": 195.77, "error": ""}, {"at": "2026-06-05T14:22:11.297682+00:00", "event": "source_run_started", "target": "__all_targets__", "source": "G1", "status": "running", "candidatesSeen": null, "candidatesTotal": null, "articlesInserted": null, "mentionsInserted": null, "storiesTouched": null, "rssBefore": 194.36, "rssAfter": null, "error": ""}, {"at": "2026-06-05T14:22:11.145279+00:00", "event": "source_run_complete", "target": "__all_targets__", "source": "Veja Rio", "status": "complete", "candidatesSeen": 0, "candidatesTotal": 0, "articlesInserted": 0, "mentionsInserted": 0, "storiesTouched": 0, "rssBefore": 193.43, "rssAfter": 194.36, "error": ""}, {"at": "2026-06-05T14:22:10.901594+00:00", "event": "source_run_started", "target": "__all_targets__", "source": "Veja Rio", "status": "running", "candidatesSeen": null, "candidatesTotal": null, "articlesInserted": null, "mentionsInserted": null, "storiesTouched": null, "rssBefore": 193.43, "rssAfter": null, "error": ""}, {"at": "2026-06-05T14:22:10.750875+00:00", "event": "source_run_complete", "target": "__all_targets__", "source": "VEJA Cidades", "status": "complete", "candidatesSeen": 0, "candidatesTotal": 0, "articlesInserted": 0, "mentionsInserted": 0, "storiesTouched": 0, "rssBefore": 194.43, "rssAfter": 194.43, "error": ""}]`.
- Barriers: `[]`.

## 2026-06-05 11:31:08 -03 - Backend Review: Sitemap Query Prefilter Subcount Fix

- User concern: story/news count is still too low for 18 broad terms over a 12-year backfill.
- Backend finding: `collect_sitemap_daily()` prefiltered sitemap entries by URL/title before fetching article bodies, but `_matches_queries()` used `normalize_text()` only. That function preserves case, accents, and punctuation, so examples like `Segurança pública` vs query `segurança`, `Crime` vs `crime`, and URL slug `reforco-no-policiamento` vs query `reforço no policiamento` failed before ingestion.
- Impact: the historical sitemap phase could discard valid candidate articles before full-text matching, especially for capitalized titles, unaccented slugs, and phrase targets with spaces/hyphens. This is a direct backend explanation for undercount risk.
- Local fix prepared: `_matches_queries()` now folds text/query with `casefold`, removes diacritics, and treats punctuation as spaces before substring matching.
- Local proof before deploy: `_matches_queries("Segurança pública no Rio", ["segurança"])`, `_matches_queries("seguranca publica no Rio", ["segurança"])`, `_matches_queries("percepcao-de-seguranca", ["percepção de segurança"])`, and `_matches_queries("Crime no Rio", ["crime"])` now return `True`.
- Tests: `.venv_playwright/bin/python -m pytest tests/test_targets_jobs.py tests/test_voluntarios_backfill_operator.py -q` passed with `85 passed`.
- Production state before deploying this fix: job `0b36e332911a` running grouped mode, `sourceRunCounts={"complete": 12, "running": 1, "pending": 22898}`, `sourceRunTargetCounts={"__all_targets__": 22911}`, current source `Estadao`, publishedAt `2026-06-05T14:29:39.819852+00:00`.
- Next action: deploy this prefilter fix, allow production to interrupt cleanly, resume only job `0b36e332911a`, and verify the sitemap phase uses the corrected matching before judging final volume.

## 2026-06-05 11:53:56 -03 - Google News Barrier: Disable Prefetch/Force-Fetch

- Post-prefilter deploy result: production interrupted cleanly, `/api/update/resume` resumed only job `0b36e332911a`, and grouped ledger remigrated to `sourceRunTargetCounts={"__all_targets__": 22911}`.
- Progress evidence after resume: RSS sources advanced again; `G1 Politica` completed with `54/54`, `21` articles, `26` mentions, `21` stories; `G1 Rio` completed with `100/100`; queue reached `Google News` at `sourceRunCounts={"complete": 18, "running": 1, "pending": 22892}`.
- Barrier: `Google News` stayed at `3/3` candidates for repeated cycles with no article/live/publishedAt movement. Current production state before patch: status `running`, source `Google News`, seen `3`, total `3`, articles `89`, publishedAt `2026-06-05T14:47:46.908328+00:00`.
- Backend diagnosis: `process_candidates()` still allowed Google News candidates into full-fetch paths through prefetch scheduling and `force_full_fetch` metadata before/around the preview-only skip. This can leave the source-run blocked even when the eventual policy is to save/skip from preview.
- Local fix prepared: `candidate_needs_prefetch()` now returns `False` for `candidate_source_type == "google_news"`, and `process_candidates()` ignores `force_full_fetch` for Google News during ingestion.
- Test hardening: the Google News preview-match test now sets `metadata={"force_full_fetch": True}`, uses `candidate_workers=2`, and monkeypatches `ThreadPoolExecutor` to fail if Google News is scheduled for prefetch.
- Tests: `.venv_playwright/bin/python -m pytest tests/test_targets_jobs.py tests/test_voluntarios_backfill_operator.py -q` passed with `85 passed`.
- Next action: deploy this Google News no-prefetch patch, let production interrupt cleanly, resume only job `0b36e332911a`, and verify `Google News` completes and advances to the six `wordpress_api` grouped source-runs.

## 2026-06-05 12:13:17 -03 - Google News Grouped Circuit Breaker

- Post no-prefetch deploy result: production interrupted cleanly, `/api/update/resume` resumed only job `0b36e332911a`, grouped ledger remigrated, RSS advanced again, and the queue reached `Google News`.
- Barrier persisted: with the no-prefetch patch deployed, production still stayed at `sourceRunCounts={"complete": 18, "running": 1, "pending": 22892}`, `sourceRunTargetCounts={"__all_targets__": 22911}`, source `Google News`, seen `3`, total `3`, publishedAt `2026-06-05T15:08:38.155084+00:00`.
- Operational decision: Google News is now treated as an unstable grouped backfill source for this production run. The source returns only `3` candidates in this long date-window state and has repeatedly blocked access to the high-volume `wordpress_api`, `internal_search`, and `sitemap_daily` phases.
- Local fix prepared: `run_source_run()` now applies a grouped-only circuit breaker for `source_type == "google_news"` and `target_key == "__all_targets__"`. It records a `source_progress` event with status `google_news_grouped_circuit_breaker`, marks candidates seen, inserts zero articles, and completes the source-run without calling `process_candidates()`.
- Scope guard: the circuit breaker is limited to grouped durable source-runs, not all Google News jobs.
- Test: `test_grouped_google_news_source_run_circuit_breaker_skips_ingest` verifies grouped Google News does not call `process_candidates()`, marks the source-run complete, records candidates seen, and emits the circuit-breaker event.
- Tests: `.venv_playwright/bin/python -m pytest tests/test_targets_jobs.py tests/test_voluntarios_backfill_operator.py -q` passed with `86 passed`.
- Next action: deploy this circuit breaker, allow production to interrupt cleanly, resume only job `0b36e332911a`, and verify the job advances from `Google News` to the six grouped `wordpress_api` source-runs.

## 2026-06-05 14:30:55 -03 - WordPress 502 Late-Page Recovery Patch

- Post Google circuit-breaker result: job `0b36e332911a` advanced past Google News. Evidence: `source_run_complete` for `Google News` at `2026-06-05T15:46:02Z`, seen `776`, total `776`, articles `0`, mentions `0`, stories `0`.
- WordPress progress: grouped `Diario do Rio` produced large checkpoints before failing: `313` candidates with `285` articles/`566` mentions/`294` stories, then `379` candidates with `244` articles/`480` mentions/`252` stories.
- Barrier: `Diario do Rio` grouped `wordpress_api` failed at `2026-06-05T17:00:02Z` with `HTTP Error 502: Bad Gateway`, causing `sourceRunCounts={"complete": 19, "failed_needs_fix": 1, "running": 1, "pending": 22890}` while `Tempo Real RJ` continued running.
- Current production state before patch: status `running`, coverage `failed_needs_fix`, source `Tempo Real RJ`, seen `210`, total `311`, articles `1654`, mentions `2780`, stories `2132`, publishedAt `2026-06-05T16:59:05.060889+00:00`.
- Backend diagnosis: a late-page WordPress 502/503/504 after substantial progress is a transient source limit, not a durable pipeline failure. Existing code already treats late hard timeouts this way; HTTP 502 needs the same recovery path.
- Local fix prepared: `collect_source_run_candidates()` and `collect_grouped_wordpress_source_run()` now treat late WordPress `HTTPError` 502/503/504 as source/query exhaustion when `page >= 5` and prior candidates are at least `WORDPRESS_PAGE_SIZE * 4`.
- Test: `test_late_grouped_wordpress_502_completes_query` reproduces a `Diario do Rio` page-16 502 after `379` candidates and asserts the grouped query completes instead of failing.
- Tests: `.venv_playwright/bin/python -m pytest tests/test_targets_jobs.py tests/test_voluntarios_backfill_operator.py -q` passed with `87 passed`.
- Next action: deploy this WordPress 502 recovery patch, resume only job `0b36e332911a`, verify no duplicate job, and confirm the failed `Diario do Rio` source-run no longer blocks final coverage.

## 2026-06-05 11:35:50 -03 - Resumed Same Production Job
...
- Current target/source: `n/a` / `n/a`.
- Totals/status fields: articles `None`, mentions `805`, stories `301`, publishedAt `2026-06-03T01:40:39.522579+00:00`.
- Website asset: generatedAt `05/06/2026 14:31 UTC`, stories `1437`, articles `1895`, targets `27`.
- Live results: latest `2026-06-03T16:51:31.525271+00:00`, count `51`.
- Viewer profile/login: login ok `True`, profile `{'found': True, 'label': 'Voluntários-Lab-Políticas-Públicas', 'hasPassword': True, 'targetKeysCount': 18, 'defaultTargetsCount': 18, 'missing': [], 'extra': [], 'defaultsMissing': [], 'defaultsExtra': [], 'targetKeys': ['seguranca_presente', 'programa_seguranca_presente', 'operacao_seguranca_presente', 'seguranca', 'inseguranca', 'crime', 'criminalidade', 'violencia', 'assalto', 'roubo', 'furto', 'medo', 'policiamento', 'patrulhamento', 'percepcao_de_seguranca', 'sensacao_de_seguranca', 'reforco_no_policiamento', 'ordem_publica'], 'defaultTargets': ['seguranca_presente', 'programa_seguranca_presente', 'operacao_seguranca_presente', 'seguranca', 'inseguranca', 'crime', 'criminalidade', 'violencia', 'assalto', 'roubo', 'furto', 'medo', 'policiamento', 'patrulhamento', 'percepcao_de_seguranca', 'sensacao_de_seguranca', 'reforco_no_policiamento', 'ordem_publica']}`.
- Target contract: `{'count': 18, 'missing': [], 'extra': [], 'primaryExact': True}`.
- Memory: VmRSS `181.46` MiB, VmHWM `682.27` MiB, limit `512` MiB.
- Disk: free `52521.68` MiB, DB `254.17` MiB.
- Storage diagnostic: enabled `False`, reason `supabase_env_not_available_locally`.
- HTTP timings/status: `{"asset": {"elapsed": 0.55, "status": 200}, "disk": {"elapsed": 0.23, "status": 200}, "events": {"elapsed": 0.23, "status": 200}, "health": {"elapsed": 1.71, "status": 200}, "live": {"elapsed": 0.44, "status": 200}, "memory": {"elapsed": 0.23, "status": 200}, "sqlite": {"elapsed": 0.67, "status": 200}, "status": {"elapsed": 1.73, "status": 200}, "targets": {"elapsed": 0.22, "status": 200}, "viewers": {"elapsed": 0.23, "status": 200}}`.
- Recent source events: `[{"at": "2026-06-03T01:41:53.755590+00:00", "event": "source_run_started", "target": "seguranca_presente", "source": "Veja Rio", "status": "running", "candidatesSeen": null, "candidatesTotal": null, "articlesInserted": null, "mentionsInserted": null, "storiesTouched": null, "rssBefore": 239.91, "rssAfter": null, "error": ""}, {"at": "2026-06-03T01:41:40.247251+00:00", "event": "source_run_checkpoint", "target": "seguranca_presente", "source": "Veja Rio", "status": "pending", "candidatesSeen": 25, "candidatesTotal": 25, "articlesInserted": 4, "mentionsInserted": 4, "storiesTouched": 4, "rssBefore": 239.91, "rssAfter": 239.91, "error": ""}, {"at": "2026-06-03T01:40:52.964517+00:00", "event": "source_run_started", "target": "seguranca_presente", "source": "Veja Rio", "status": "running", "candidatesSeen": null, "candidatesTotal": null, "articlesInserted": null, "mentionsInserted": null, "storiesTouched": null, "rssBefore": 239.91, "rssAfter": null, "error": ""}, {"at": "2026-06-03T01:40:22.446931+00:00", "event": "source_run_checkpoint", "target": "seguranca_presente", "source": "Veja Rio", "status": "pending", "candidatesSeen": 25, "candidatesTotal": 25, "articlesInserted": 10, "mentionsInserted": 10, "storiesTouched": 10, "rssBefore": 239.91, "rssAfter": 239.91, "error": ""}, {"at": "2026-06-03T01:39:08.444608+00:00", "event": "source_run_started", "target": "seguranca_presente", "source": "Veja Rio", "status": "running", "candidatesSeen": null, "candidatesTotal": null, "articlesInserted": null, "mentionsInserted": null, "storiesTouched": null, "rssBefore": 239.91, "rssAfter": null, "error": ""}, {"at": "2026-06-03T01:38:38.069488+00:00", "event": "source_run_complete", "target": "seguranca_presente", "source": "Tribuna da Serra", "status": "complete", "candidatesSeen": 24, "candidatesTotal": 24, "articlesInserted": 8, "mentionsInserted": 8, "storiesTouched": 8, "rssBefore": 239.91, "rssAfter": 239.91, "error": ""}, {"at": "2026-06-03T01:36:57.604497+00:00", "event": "source_run_started", "target": "seguranca_presente", "source": "Tribuna da Serra", "status": "running", "candidatesSeen": null, "candidatesTotal": null, "articlesInserted": null, "mentionsInserted": null, "storiesTouched": null, "rssBefore": 239.91, "rssAfter": null, "error": ""}, {"at": "2026-06-03T01:36:25.606763+00:00", "event": "source_run_checkpoint", "target": "seguranca_presente", "source": "Tribuna da Serra", "status": "pending", "candidatesSeen": 25, "candidatesTotal": 25, "articlesInserted": 4, "mentionsInserted": 4, "storiesTouched": 4, "rssBefore": 239.91, "rssAfter": 239.91, "error": ""}]`.
- Barriers: `[]`.

## 2026-06-05 11:58:03 -03 - Resumed Same Production Job
...
- Current target/source: `n/a` / `n/a`.
- Totals/status fields: articles `None`, mentions `805`, stories `301`, publishedAt `2026-06-03T01:40:39.522579+00:00`.
- Website asset: generatedAt `05/06/2026 14:47 UTC`, stories `1437`, articles `1895`, targets `27`.
- Live results: latest `2026-06-03T16:51:31.525271+00:00`, count `51`.
- Viewer profile/login: login ok `True`, profile `{'found': True, 'label': 'Voluntários-Lab-Políticas-Públicas', 'hasPassword': True, 'targetKeysCount': 18, 'defaultTargetsCount': 18, 'missing': [], 'extra': [], 'defaultsMissing': [], 'defaultsExtra': [], 'targetKeys': ['seguranca_presente', 'programa_seguranca_presente', 'operacao_seguranca_presente', 'seguranca', 'inseguranca', 'crime', 'criminalidade', 'violencia', 'assalto', 'roubo', 'furto', 'medo', 'policiamento', 'patrulhamento', 'percepcao_de_seguranca', 'sensacao_de_seguranca', 'reforco_no_policiamento', 'ordem_publica'], 'defaultTargets': ['seguranca_presente', 'programa_seguranca_presente', 'operacao_seguranca_presente', 'seguranca', 'inseguranca', 'crime', 'criminalidade', 'violencia', 'assalto', 'roubo', 'furto', 'medo', 'policiamento', 'patrulhamento', 'percepcao_de_seguranca', 'sensacao_de_seguranca', 'reforco_no_policiamento', 'ordem_publica']}`.
- Target contract: `{'count': 18, 'missing': [], 'extra': [], 'primaryExact': True}`.
- Memory: VmRSS `173.74` MiB, VmHWM `675.8` MiB, limit `512` MiB.
- Disk: free `55255.42` MiB, DB `254.17` MiB.
- Storage diagnostic: enabled `False`, reason `supabase_env_not_available_locally`.
- HTTP timings/status: `{"asset": {"elapsed": 1.06, "status": 200}, "disk": {"elapsed": 0.55, "status": 200}, "events": {"elapsed": 0.25, "status": 200}, "health": {"elapsed": 1.79, "status": 200}, "live": {"elapsed": 0.29, "status": 200}, "memory": {"elapsed": 0.59, "status": 200}, "sqlite": {"elapsed": 0.83, "status": 200}, "status": {"elapsed": 1.89, "status": 200}, "targets": {"elapsed": 0.23, "status": 200}, "viewers": {"elapsed": 0.24, "status": 200}}`.
- Recent source events: `[{"at": "2026-06-03T01:41:53.755590+00:00", "event": "source_run_started", "target": "seguranca_presente", "source": "Veja Rio", "status": "running", "candidatesSeen": null, "candidatesTotal": null, "articlesInserted": null, "mentionsInserted": null, "storiesTouched": null, "rssBefore": 239.91, "rssAfter": null, "error": ""}, {"at": "2026-06-03T01:41:40.247251+00:00", "event": "source_run_checkpoint", "target": "seguranca_presente", "source": "Veja Rio", "status": "pending", "candidatesSeen": 25, "candidatesTotal": 25, "articlesInserted": 4, "mentionsInserted": 4, "storiesTouched": 4, "rssBefore": 239.91, "rssAfter": 239.91, "error": ""}, {"at": "2026-06-03T01:40:52.964517+00:00", "event": "source_run_started", "target": "seguranca_presente", "source": "Veja Rio", "status": "running", "candidatesSeen": null, "candidatesTotal": null, "articlesInserted": null, "mentionsInserted": null, "storiesTouched": null, "rssBefore": 239.91, "rssAfter": null, "error": ""}, {"at": "2026-06-03T01:40:22.446931+00:00", "event": "source_run_checkpoint", "target": "seguranca_presente", "source": "Veja Rio", "status": "pending", "candidatesSeen": 25, "candidatesTotal": 25, "articlesInserted": 10, "mentionsInserted": 10, "storiesTouched": 10, "rssBefore": 239.91, "rssAfter": 239.91, "error": ""}, {"at": "2026-06-03T01:39:08.444608+00:00", "event": "source_run_started", "target": "seguranca_presente", "source": "Veja Rio", "status": "running", "candidatesSeen": null, "candidatesTotal": null, "articlesInserted": null, "mentionsInserted": null, "storiesTouched": null, "rssBefore": 239.91, "rssAfter": null, "error": ""}, {"at": "2026-06-03T01:38:38.069488+00:00", "event": "source_run_complete", "target": "seguranca_presente", "source": "Tribuna da Serra", "status": "complete", "candidatesSeen": 24, "candidatesTotal": 24, "articlesInserted": 8, "mentionsInserted": 8, "storiesTouched": 8, "rssBefore": 239.91, "rssAfter": 239.91, "error": ""}, {"at": "2026-06-03T01:36:57.604497+00:00", "event": "source_run_started", "target": "seguranca_presente", "source": "Tribuna da Serra", "status": "running", "candidatesSeen": null, "candidatesTotal": null, "articlesInserted": null, "mentionsInserted": null, "storiesTouched": null, "rssBefore": 239.91, "rssAfter": null, "error": ""}, {"at": "2026-06-03T01:36:25.606763+00:00", "event": "source_run_checkpoint", "target": "seguranca_presente", "source": "Tribuna da Serra", "status": "pending", "candidatesSeen": 25, "candidatesTotal": 25, "articlesInserted": 4, "mentionsInserted": 4, "storiesTouched": 4, "rssBefore": 239.91, "rssAfter": 239.91, "error": ""}]`.
- Barriers: `[]`.

## 2026-06-05 12:17:18 -03 - Resumed Same Production Job
...
- Current target/source: `n/a` / `n/a`.
- Totals/status fields: articles `None`, mentions `805`, stories `301`, publishedAt `2026-06-03T01:40:39.522579+00:00`.
- Website asset: generatedAt `05/06/2026 15:08 UTC`, stories `1437`, articles `1895`, targets `27`.
- Live results: latest `2026-06-03T16:51:31.525271+00:00`, count `51`.
- Viewer profile/login: login ok `True`, profile `{'found': True, 'label': 'Voluntários-Lab-Políticas-Públicas', 'hasPassword': True, 'targetKeysCount': 18, 'defaultTargetsCount': 18, 'missing': [], 'extra': [], 'defaultsMissing': [], 'defaultsExtra': [], 'targetKeys': ['seguranca_presente', 'programa_seguranca_presente', 'operacao_seguranca_presente', 'seguranca', 'inseguranca', 'crime', 'criminalidade', 'violencia', 'assalto', 'roubo', 'furto', 'medo', 'policiamento', 'patrulhamento', 'percepcao_de_seguranca', 'sensacao_de_seguranca', 'reforco_no_policiamento', 'ordem_publica'], 'defaultTargets': ['seguranca_presente', 'programa_seguranca_presente', 'operacao_seguranca_presente', 'seguranca', 'inseguranca', 'crime', 'criminalidade', 'violencia', 'assalto', 'roubo', 'furto', 'medo', 'policiamento', 'patrulhamento', 'percepcao_de_seguranca', 'sensacao_de_seguranca', 'reforco_no_policiamento', 'ordem_publica']}`.
- Target contract: `{'count': 18, 'missing': [], 'extra': [], 'primaryExact': True}`.
- Memory: VmRSS `166.52` MiB, VmHWM `676.21` MiB, limit `512` MiB.
- Disk: free `60892.5` MiB, DB `254.17` MiB.
- Storage diagnostic: enabled `False`, reason `supabase_env_not_available_locally`.
- HTTP timings/status: `{"asset": {"elapsed": 0.58, "status": 200}, "disk": {"elapsed": 0.42, "status": 200}, "events": {"elapsed": 0.38, "status": 200}, "health": {"elapsed": 1.84, "status": 200}, "live": {"elapsed": 0.3, "status": 200}, "memory": {"elapsed": 0.5, "status": 200}, "sqlite": {"elapsed": 0.82, "status": 200}, "status": {"elapsed": 1.54, "status": 200}, "targets": {"elapsed": 0.23, "status": 200}, "viewers": {"elapsed": 0.23, "status": 200}}`.
- Recent source events: `[{"at": "2026-06-03T01:41:53.755590+00:00", "event": "source_run_started", "target": "seguranca_presente", "source": "Veja Rio", "status": "running", "candidatesSeen": null, "candidatesTotal": null, "articlesInserted": null, "mentionsInserted": null, "storiesTouched": null, "rssBefore": 239.91, "rssAfter": null, "error": ""}, {"at": "2026-06-03T01:41:40.247251+00:00", "event": "source_run_checkpoint", "target": "seguranca_presente", "source": "Veja Rio", "status": "pending", "candidatesSeen": 25, "candidatesTotal": 25, "articlesInserted": 4, "mentionsInserted": 4, "storiesTouched": 4, "rssBefore": 239.91, "rssAfter": 239.91, "error": ""}, {"at": "2026-06-03T01:40:52.964517+00:00", "event": "source_run_started", "target": "seguranca_presente", "source": "Veja Rio", "status": "running", "candidatesSeen": null, "candidatesTotal": null, "articlesInserted": null, "mentionsInserted": null, "storiesTouched": null, "rssBefore": 239.91, "rssAfter": null, "error": ""}, {"at": "2026-06-03T01:40:22.446931+00:00", "event": "source_run_checkpoint", "target": "seguranca_presente", "source": "Veja Rio", "status": "pending", "candidatesSeen": 25, "candidatesTotal": 25, "articlesInserted": 10, "mentionsInserted": 10, "storiesTouched": 10, "rssBefore": 239.91, "rssAfter": 239.91, "error": ""}, {"at": "2026-06-03T01:39:08.444608+00:00", "event": "source_run_started", "target": "seguranca_presente", "source": "Veja Rio", "status": "running", "candidatesSeen": null, "candidatesTotal": null, "articlesInserted": null, "mentionsInserted": null, "storiesTouched": null, "rssBefore": 239.91, "rssAfter": null, "error": ""}, {"at": "2026-06-03T01:38:38.069488+00:00", "event": "source_run_complete", "target": "seguranca_presente", "source": "Tribuna da Serra", "status": "complete", "candidatesSeen": 24, "candidatesTotal": 24, "articlesInserted": 8, "mentionsInserted": 8, "storiesTouched": 8, "rssBefore": 239.91, "rssAfter": 239.91, "error": ""}, {"at": "2026-06-03T01:36:57.604497+00:00", "event": "source_run_started", "target": "seguranca_presente", "source": "Tribuna da Serra", "status": "running", "candidatesSeen": null, "candidatesTotal": null, "articlesInserted": null, "mentionsInserted": null, "storiesTouched": null, "rssBefore": 239.91, "rssAfter": null, "error": ""}, {"at": "2026-06-03T01:36:25.606763+00:00", "event": "source_run_checkpoint", "target": "seguranca_presente", "source": "Tribuna da Serra", "status": "pending", "candidatesSeen": 25, "candidatesTotal": 25, "articlesInserted": 4, "mentionsInserted": 4, "storiesTouched": 4, "rssBefore": 239.91, "rssAfter": 239.91, "error": ""}]`.
- Barriers: `[]`.

## 2026-06-05 14:34:29 -03 - Resumed Same Production Job
...
- Current target/source: `n/a` / `n/a`.
- Totals/status fields: articles `None`, mentions `805`, stories `301`, publishedAt `2026-06-03T01:40:39.522579+00:00`.
- Website asset: generatedAt `05/06/2026 16:58 UTC`, stories `1787`, articles `2319`, targets `27`.
- Live results: latest `2026-06-03T16:51:31.525271+00:00`, count `51`.
- Viewer profile/login: login ok `True`, profile `{'found': True, 'label': 'Voluntários-Lab-Políticas-Públicas', 'hasPassword': True, 'targetKeysCount': 18, 'defaultTargetsCount': 18, 'missing': [], 'extra': [], 'defaultsMissing': [], 'defaultsExtra': [], 'targetKeys': ['seguranca_presente', 'programa_seguranca_presente', 'operacao_seguranca_presente', 'seguranca', 'inseguranca', 'crime', 'criminalidade', 'violencia', 'assalto', 'roubo', 'furto', 'medo', 'policiamento', 'patrulhamento', 'percepcao_de_seguranca', 'sensacao_de_seguranca', 'reforco_no_policiamento', 'ordem_publica'], 'defaultTargets': ['seguranca_presente', 'programa_seguranca_presente', 'operacao_seguranca_presente', 'seguranca', 'inseguranca', 'crime', 'criminalidade', 'violencia', 'assalto', 'roubo', 'furto', 'medo', 'policiamento', 'patrulhamento', 'percepcao_de_seguranca', 'sensacao_de_seguranca', 'reforco_no_policiamento', 'ordem_publica']}`.
- Target contract: `{'count': 18, 'missing': [], 'extra': [], 'primaryExact': True}`.
- Memory: VmRSS `186.02` MiB, VmHWM `682.42` MiB, limit `512` MiB.
- Disk: free `44124.6` MiB, DB `254.17` MiB.
- Storage diagnostic: enabled `False`, reason `supabase_env_not_available_locally`.
- HTTP timings/status: `{"asset": {"elapsed": 0.68, "status": 200}, "disk": {"elapsed": 0.23, "status": 200}, "events": {"elapsed": 0.27, "status": 200}, "health": {"elapsed": 1.43, "status": 200}, "live": {"elapsed": 0.29, "status": 200}, "memory": {"elapsed": 0.22, "status": 200}, "sqlite": {"elapsed": 1.09, "status": 200}, "status": {"elapsed": 1.69, "status": 200}, "targets": {"elapsed": 0.23, "status": 200}, "viewers": {"elapsed": 0.24, "status": 200}}`.
- Recent source events: `[{"at": "2026-06-03T01:41:53.755590+00:00", "event": "source_run_started", "target": "seguranca_presente", "source": "Veja Rio", "status": "running", "candidatesSeen": null, "candidatesTotal": null, "articlesInserted": null, "mentionsInserted": null, "storiesTouched": null, "rssBefore": 239.91, "rssAfter": null, "error": ""}, {"at": "2026-06-03T01:41:40.247251+00:00", "event": "source_run_checkpoint", "target": "seguranca_presente", "source": "Veja Rio", "status": "pending", "candidatesSeen": 25, "candidatesTotal": 25, "articlesInserted": 4, "mentionsInserted": 4, "storiesTouched": 4, "rssBefore": 239.91, "rssAfter": 239.91, "error": ""}, {"at": "2026-06-03T01:40:52.964517+00:00", "event": "source_run_started", "target": "seguranca_presente", "source": "Veja Rio", "status": "running", "candidatesSeen": null, "candidatesTotal": null, "articlesInserted": null, "mentionsInserted": null, "storiesTouched": null, "rssBefore": 239.91, "rssAfter": null, "error": ""}, {"at": "2026-06-03T01:40:22.446931+00:00", "event": "source_run_checkpoint", "target": "seguranca_presente", "source": "Veja Rio", "status": "pending", "candidatesSeen": 25, "candidatesTotal": 25, "articlesInserted": 10, "mentionsInserted": 10, "storiesTouched": 10, "rssBefore": 239.91, "rssAfter": 239.91, "error": ""}, {"at": "2026-06-03T01:39:08.444608+00:00", "event": "source_run_started", "target": "seguranca_presente", "source": "Veja Rio", "status": "running", "candidatesSeen": null, "candidatesTotal": null, "articlesInserted": null, "mentionsInserted": null, "storiesTouched": null, "rssBefore": 239.91, "rssAfter": null, "error": ""}, {"at": "2026-06-03T01:38:38.069488+00:00", "event": "source_run_complete", "target": "seguranca_presente", "source": "Tribuna da Serra", "status": "complete", "candidatesSeen": 24, "candidatesTotal": 24, "articlesInserted": 8, "mentionsInserted": 8, "storiesTouched": 8, "rssBefore": 239.91, "rssAfter": 239.91, "error": ""}, {"at": "2026-06-03T01:36:57.604497+00:00", "event": "source_run_started", "target": "seguranca_presente", "source": "Tribuna da Serra", "status": "running", "candidatesSeen": null, "candidatesTotal": null, "articlesInserted": null, "mentionsInserted": null, "storiesTouched": null, "rssBefore": 239.91, "rssAfter": null, "error": ""}, {"at": "2026-06-03T01:36:25.606763+00:00", "event": "source_run_checkpoint", "target": "seguranca_presente", "source": "Tribuna da Serra", "status": "pending", "candidatesSeen": 25, "candidatesTotal": 25, "articlesInserted": 4, "mentionsInserted": 4, "storiesTouched": 4, "rssBefore": 239.91, "rssAfter": 239.91, "error": ""}]`.
- Barriers: `[]`.

## 2026-06-05 15:08:46 -03 - Resumed Same Production Job
...
- Current target/source: `n/a` / `n/a`.
- Totals/status fields: articles `None`, mentions `805`, stories `301`, publishedAt `2026-06-03T01:40:39.522579+00:00`.
- Website asset: generatedAt `05/06/2026 17:45 UTC`, stories `1785`, articles `2276`, targets `27`.
- Live results: latest `2026-06-03T16:51:31.525271+00:00`, count `51`.
- Viewer profile/login: login ok `True`, profile `{'found': True, 'label': 'Voluntários-Lab-Políticas-Públicas', 'hasPassword': True, 'targetKeysCount': 18, 'defaultTargetsCount': 18, 'missing': [], 'extra': [], 'defaultsMissing': [], 'defaultsExtra': [], 'targetKeys': ['seguranca_presente', 'programa_seguranca_presente', 'operacao_seguranca_presente', 'seguranca', 'inseguranca', 'crime', 'criminalidade', 'violencia', 'assalto', 'roubo', 'furto', 'medo', 'policiamento', 'patrulhamento', 'percepcao_de_seguranca', 'sensacao_de_seguranca', 'reforco_no_policiamento', 'ordem_publica'], 'defaultTargets': ['seguranca_presente', 'programa_seguranca_presente', 'operacao_seguranca_presente', 'seguranca', 'inseguranca', 'crime', 'criminalidade', 'violencia', 'assalto', 'roubo', 'furto', 'medo', 'policiamento', 'patrulhamento', 'percepcao_de_seguranca', 'sensacao_de_seguranca', 'reforco_no_policiamento', 'ordem_publica']}`.
- Target contract: `{'count': 18, 'missing': [], 'extra': [], 'primaryExact': True}`.
- Memory: VmRSS `190.05` MiB, VmHWM `676.34` MiB, limit `512` MiB.
- Disk: free `50534.63` MiB, DB `254.17` MiB.
- Storage diagnostic: enabled `False`, reason `supabase_env_not_available_locally`.
- HTTP timings/status: `{"asset": {"elapsed": 0.57, "status": 200}, "disk": {"elapsed": 0.24, "status": 200}, "events": {"elapsed": 0.23, "status": 200}, "health": {"elapsed": 1.29, "status": 200}, "live": {"elapsed": 0.29, "status": 200}, "memory": {"elapsed": 0.22, "status": 200}, "sqlite": {"elapsed": 0.9, "status": 200}, "status": {"elapsed": 1.35, "status": 200}, "targets": {"elapsed": 0.23, "status": 200}, "viewers": {"elapsed": 0.24, "status": 200}}`.
- Recent source events: `[{"at": "2026-06-03T01:41:53.755590+00:00", "event": "source_run_started", "target": "seguranca_presente", "source": "Veja Rio", "status": "running", "candidatesSeen": null, "candidatesTotal": null, "articlesInserted": null, "mentionsInserted": null, "storiesTouched": null, "rssBefore": 239.91, "rssAfter": null, "error": ""}, {"at": "2026-06-03T01:41:40.247251+00:00", "event": "source_run_checkpoint", "target": "seguranca_presente", "source": "Veja Rio", "status": "pending", "candidatesSeen": 25, "candidatesTotal": 25, "articlesInserted": 4, "mentionsInserted": 4, "storiesTouched": 4, "rssBefore": 239.91, "rssAfter": 239.91, "error": ""}, {"at": "2026-06-03T01:40:52.964517+00:00", "event": "source_run_started", "target": "seguranca_presente", "source": "Veja Rio", "status": "running", "candidatesSeen": null, "candidatesTotal": null, "articlesInserted": null, "mentionsInserted": null, "storiesTouched": null, "rssBefore": 239.91, "rssAfter": null, "error": ""}, {"at": "2026-06-03T01:40:22.446931+00:00", "event": "source_run_checkpoint", "target": "seguranca_presente", "source": "Veja Rio", "status": "pending", "candidatesSeen": 25, "candidatesTotal": 25, "articlesInserted": 10, "mentionsInserted": 10, "storiesTouched": 10, "rssBefore": 239.91, "rssAfter": 239.91, "error": ""}, {"at": "2026-06-03T01:39:08.444608+00:00", "event": "source_run_started", "target": "seguranca_presente", "source": "Veja Rio", "status": "running", "candidatesSeen": null, "candidatesTotal": null, "articlesInserted": null, "mentionsInserted": null, "storiesTouched": null, "rssBefore": 239.91, "rssAfter": null, "error": ""}, {"at": "2026-06-03T01:38:38.069488+00:00", "event": "source_run_complete", "target": "seguranca_presente", "source": "Tribuna da Serra", "status": "complete", "candidatesSeen": 24, "candidatesTotal": 24, "articlesInserted": 8, "mentionsInserted": 8, "storiesTouched": 8, "rssBefore": 239.91, "rssAfter": 239.91, "error": ""}, {"at": "2026-06-03T01:36:57.604497+00:00", "event": "source_run_started", "target": "seguranca_presente", "source": "Tribuna da Serra", "status": "running", "candidatesSeen": null, "candidatesTotal": null, "articlesInserted": null, "mentionsInserted": null, "storiesTouched": null, "rssBefore": 239.91, "rssAfter": null, "error": ""}, {"at": "2026-06-03T01:36:25.606763+00:00", "event": "source_run_checkpoint", "target": "seguranca_presente", "source": "Tribuna da Serra", "status": "pending", "candidatesSeen": 25, "candidatesTotal": 25, "articlesInserted": 4, "mentionsInserted": 4, "storiesTouched": 4, "rssBefore": 239.91, "rssAfter": 239.91, "error": ""}]`.
- Barriers: `[]`.
